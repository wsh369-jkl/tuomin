...
class ReviewEvidenceRef(BaseModel):
    quote: str = Field(default="", description="Evidence snippet")
    start: int = Field(default=0, description="Start offset in the normalized document text")
    end: int = Field(default=0, description="End offset in the normalized document text")
    page: Optional[int] = Field(default=None, description="Page number when available")
    block_type: str = Field(default="text", description="Source block type")
    evidence_quality: str = Field(default="high", description="Evidence quality tier")


class ReviewCardItem(BaseModel):
    title: Optional[str] = Field(default=None, description="Short item title")
    label: Optional[str] = Field(default=None, description="Label shown in structured cards")
    value: Optional[str] = Field(default=None, description="Primary item value")
    severity: Optional[str] = Field(default=None, description="Severity level when applicable")
    reason: Optional[str] = Field(default=None, description="Why this item matters")
    evidence_refs: List[ReviewEvidenceRef] = Field(default_factory=list, description="Evidence references")
    lawyer_action_hint: Optional[str] = Field(default=None, description="Suggested follow-up action")


class ReviewCard(BaseModel):
    type: str = Field(..., description="Card type")
    title: str = Field(..., description="Card title")
    items: List[ReviewCardItem] = Field(default_factory=list, description="Card items")


class ReviewGenerateRequest(BaseModel):
    task_id: str = Field(..., description="Analysis task ID")
    llm_model: Optional[str] = Field(default=None, description="Requested review model")


class ReviewResult(BaseModel):
    review_id: str = Field(..., description="Review task ID")
    task_id: str = Field(..., description="Source analysis task ID")
    document_type: str = Field(default="other", description="Derived document type")
    document_type_label: str = Field(default="其他正式文书", description="Human-readable document type")
    summary: str = Field(default="", description="Top-level summary")
    cards: List[ReviewCard] = Field(default_factory=list, description="Structured review cards")
    canonical_groups: List[Dict] = Field(default_factory=list, description="Canonical identity groups")
    suspected_misses: List[Dict] = Field(default_factory=list, description="Potential misses or issues to review")
    metadata: Dict = Field(default_factory=dict, description="Review generation metadata")


class AssistantItem(BaseModel):
    title: Optional[str] = Field(default=None, description="Short item title")
    label: Optional[str] = Field(default=None, description="Label shown in the assistant section")
    value: Optional[str] = Field(default=None, description="Primary value")
    status: Optional[str] = Field(default=None, description="Status flag such as ready, missing, or warning")
    severity: Optional[str] = Field(default=None, description="Severity level when applicable")
    reason: Optional[str] = Field(default=None, description="Why this item matters")
    evidence_refs: List[ReviewEvidenceRef] = Field(default_factory=list, description="Evidence references")
    action_hint: Optional[str] = Field(default=None, description="Suggested follow-up action")


class AssistantSection(BaseModel):
    type: str = Field(..., description="Section type")
    title: str = Field(..., description="Section title")
    items: List[AssistantItem] = Field(default_factory=list, description="Section items")


class AssistantTaskStatus(BaseModel):
    assistant_id: str = Field(..., description="Assistant task ID")
    filename: Optional[str] = Field(default=None, description="Uploaded filename")
    status: str = Field(..., description="Task status")
    progress: int = Field(default=0, description="Progress from 0 to 100")
    stage_key: Optional[str] = Field(default=None, description="Current stage key")
    stage_label: Optional[str] = Field(default=None, description="Current stage label")
    message: Optional[str] = Field(default=None, description="Status message")
    error_message: Optional[str] = Field(default=None, description="Terminal error message")
    created_at: datetime = Field(default_factory=datetime.now, description="Creation time")
...
class AssistantResult(BaseModel):
    ...
    document_type: str = Field(default="other", description="Derived document type")
    document_type_label: str = Field(default="其他正式文书", description="Human-readable document type")
    support_mode: str = Field(default="limited", description="Whether the document is fully supported")
    support_notice: str = Field(default="", description="Scope or support notice")
    summary: str = Field(default="", description="Top-level summary")
    sections: List[AssistantSection] = Field(default_factory=list, description="Assistant result sections")
    text: str = Field(default="", description="Parsed source text used for preview")
    metadata: Dict = Field(default_factory=dict, description="Assistant generation metadata")
